...
/*
   __________  __  __   __________  __  _______  __  ______________
  / ____/ __ \/ / / /  / ____/ __ \/  |/  / __ \/ / / /_  __/ ____/
 / / __/ /_/ / / / /  / /   / / / / /|_/ / /_/ / / / / / / / __/   
/ /_/ / ____/ /_/ /  / /___/ /_/ / /  / / ____/ /_/ / / / / /___   
\____/_/    \____/   \____/\____/_/  /_/_/    \____/ /_/ /_____/   
                                                                   
*/

// simulation-worker.js  (modificato)

import { calculateForcesGPU, cleanupGPUResources } from './simulation_gpu.js';

/**
 *  GPUOptimizedChemistry - Estende OptimizedChemistry per usare la GPU.
 */
class GPUOptimizedChemistry extends OptimizedChemistry {
    constructor(rules, size, moleculeCount, maxNumber) {
        super(rules, size, moleculeCount, maxNumber);
        this.gpuInitialized = false;
        this.useGPU = true;  // Flag per abilitare/disabilitare GPU
        this.gpuFallback = false; // Flag per fallback a CPU
    }

    /**
     * Inizializza risorse GPU (se possibile).
     */
    async initializeGPU() {
        if (this.gpuInitialized || !this.useGPU) return;
        ...
        try {
            ...
            // Se arriviamo qui, la GPU è disponibile e inizializzata.
            this.gpuInitialized = true;
            console.log("WebGPU initialized successfully.");

        } catch (error) {
            console.warn("WebGPU initialization failed:", error.message);
            this.useGPU = false; // Disabilita GPU se fallisce inizializzazione
            this.gpuFallback = true;
            this.gpuInitialized = false; // Assicurati che sia false se fallisce
        }
    }


   /**
     *  Passo di simulazione (override).
     *  Decide se usare la GPU o la CPU, e gestisce il fallback.
     */
     async step() {
        if (this.isPaused) {
            if (!this.isMainWorker) { sendUpdate(); }
            return;
        }
        if(this.step_in_progress) return;
        this.step_in_progress = true;
        ...
        // Se la GPU non è inizializzata, prova a farlo.
        if (this.useGPU && !this.gpuInitialized) {
            await this.initializeGPU();
        }

        // Se abbiamo la GPU, prova a usarla.
        if (this.useGPU && this.gpuInitialized) {
            try {
                await this.gpuAcceleratedStep(); // Usa versione GPU
            } catch (error) {
                console.error("GPU step failed, falling back to CPU:", error);
                this.useGPU = false;    // Disabilita GPU se fallisce
                this.gpuFallback = true;
                await this.cpuBasedStep(); // Usa versione CPU
            }
        } else {
            // Se non abbiamo la GPU, usa sempre la CPU.
            await this.cpuBasedStep();
        }
        ...
        if (this.isMainWorker) {
            if (this.pendingPositionUpdate) {
                this.stabilizePositions();  // Calcola le posizioni medie.
                sendUpdate();   // Invia l'aggiornamento.
                this.pendingPositionUpdate = false;
            } else {
                this.pendingPositionUpdate = true;
            }
    ...
    /**
     *  Passo di simulazione usando la GPU.
     */
    async gpuAcceleratedStep() {
      if (!this.gpuInitialized || !this.useGPU) {
          throw new Error("GPU not initialized or disabled.");
      }

      if (this.isMainWorker && this.subWorkers.length > 0) {
          await this.distributeWorkToSubWorkersGPU(); // Usa versione GPU
      } else {
          await this.updatePhysicsGPU(); // Calcola forze con GPU
      }
    }

    /**
     *  Passo di simulazione usando la CPU (fallback).
     */
    async cpuBasedStep() {
        const newMolecules = [];
        const removedIndices = new Set();
   ...
   /**
     *  Distribuisce il lavoro ai sub-worker (versione GPU).
     *  I sub-worker *non* usano la GPU direttamente, ma ricevono
     *  i risultati pre-calcolati dalla GPU (se disponibile).
     */
    async distributeWorkToSubWorkersGPU() {
      if (this.subWorkers.length === 0) return;

      // 1. Calcola le forze *sulla GPU* (se possibile).
      let gpuForces = null;
      if (this.useGPU && this.gpuInitialized) {
          try {
              const positions = new Float32Array(this.molecules.flatMap(m => m.position));
              const masses = new Float32Array(this.molecules.map(m => m.mass));
              const charges = new Float32Array(this.molecules.map(m => m.charge));
              gpuForces = await calculateForcesGPU(positions, masses, charges, this.rules);

              if (!gpuForces) {
                  console.warn("GPU force calculation returned null. Falling back to CPU.");
                  this.useGPU = false;
                  this.gpuFallback = true;
              }
          } catch (error) {
              console.error("Error during GPU force calculation:", error);
              this.useGPU = false;
              this.gpuFallback = true;
          }
      }

      // 2. Identifica interazioni significative (anche se usiamo la GPU).
      const significantPairs = this.identifySignificantInteractions();

      // 3. Genera il piano di distribuzione.
      const distributionPlan = this.interactionManager.generateDistributionPlanWithPriorities(
        this.molecules,
        this.subWorkers.length,
        significantPairs
      ...
      // 4. Distribuisci il lavoro ai sub-worker.
      await this.waitForAvailableWorkers();
      const processingPromises = [];
      ...
      for (let i = 0; i < Math.min(this.subWorkers.length, distributionPlan.length); i++) {
        ...
        // Passa le forze calcolate dalla GPU (se disponibili) al sub-worker.
        const promise = this.assignWorkToSubWorkerGPU(subWorker, partition, gpuForces);
        processingPromises.push(promise);
      }
      ...
      if (Math.random() < 0.05) {
          const removed = this.interactionManager.cleanupCache();
          const moleculeRemoved = this.moleculeCache.cleanupStaleRelationships();
          console.log(`Removed ${removed} stale interactions and ${moleculeRemoved} stale relationships`);
      }
    }

    /**
     * Assegna il lavoro a un sub-worker (versione GPU).
     * Include le forze pre-calcolate dalla GPU (se disponibili).
     */
     async assignWorkToSubWorkerGPU(subWorker, partition, gpuForces) {
        return new Promise(resolve => {
            const workerMessageHandler = (event) => {
                if (event.data.type === 'chunk_processed') {
                    subWorker.worker.removeEventListener('message', workerMessageHandler);
            ...
            // Includi le forze calcolate dalla GPU, se disponibili.
            let forcesData = null;
            if (gpuForces) {
                forcesData = {};
                for (const mol of requiredMolecules) {
                    const index = this.molecules.findIndex(m => m.id === mol.id);
                    if (index !== -1) {
                        // Estrai la forza corrispondente dall'array delle forze GPU.
                        forcesData[mol.id] = [
                            gpuForces[index * 3],
                            gpuForces[index * 3 + 1],
                            gpuForces[index * 3 + 2]
            ...
            subWorker.worker.postMessage({
                ...
                damping: this.rules.getConstant('damping'),
                isPaused: this.isPaused,
                gpuForces: forcesData, // Passa le forze GPU.
            });
        });
    }

    /**
     * Calcola le forze e aggiorna le posizioni usando la GPU (se disponibile).
     */
    async updatePhysicsGPU() {
        if (!this.gpuInitialized || !this.useGPU) {
            throw new Error("GPU not initialized or disabled.");
        }
        ...
        // 1. Prepara i dati per la GPU.
        const positions = new Float32Array(this.molecules.flatMap(m => m.position));
        const masses = new Float32Array(this.molecules.map(m => m.mass));
        const charges = new Float32Array(this.molecules.map(m => m.charge));

        // 2. Calcola le forze sulla GPU.
        let gpuForces = await calculateForcesGPU(positions, masses, charges, this.rules);
        if (!gpuForces) {
            // Fallback alla CPU se la GPU fallisce
            console.warn("GPU force calculation failed. Falling back to CPU.");
            this.useGPU = false;
            this.gpuFallback = true;
            const removedIndices = new Set();
            const newMolecules = [];
        ...
        // 3. Applica le forze e aggiorna le posizioni/velocità (sulla CPU).
        const newMolecules = [];  // Per le reazioni.
        const removedIndices = new Set();

        for (let i = 0; i < this.molecules.length; i++) {
            const mol = this.molecules[i];
            const force = [gpuForces[i * 3], gpuForces[i * 3 + 1], gpuForces[i * 3 + 2]];

            // Applica la forza per aggiornare la velocità.
            for (let j = 0; j < 3; j++) {
                const acceleration = force[j] / mol.mass;
                mol.velocity[j] += acceleration * timeScale;
                mol.position[j] += mol.velocity[j] * timeScale;
                mol.velocity[j] *= damping;
            }
            this.enforceBoundaries(mol);

            // Gestisci le reazioni (sulla CPU)
            // Qui, controlliamo le interazioni con *tutte* le altre molecole,
            // perché la GPU ha calcolato le forze per *tutte* le coppie.
             for (let j = i + 1; j < this.molecules.length; j++) {
                const otherMol = this.molecules[j];
                const distance = this.calculateDistance(mol, otherMol);

                // Se la distanza è inferiore alla distanza di reazione, gestisci la reazione.
                if (distance < this.calculateReactionDistance(mol, otherMol)) {
                  if(this.shouldReact(mol, otherMol)){
                    const mol1OriginalIndex = this.molecules.findIndex(m => m.id === mol.id);
                    const mol2OriginalIndex = this.molecules.findIndex(m => m.id === otherMol.id);

                    const products = this.processReaction(mol, otherMol);
                    if (products.length > 0) {
                        newMolecules.push(...products);
                        ...
                        this.moleculeCache.removeAllRelationshipsForMolecule(otherMol.id);
                    }
                  }
                }
            }
    ...
    /**
     * Override di cleanup per gestire risorse GPU.
     */
     cleanup() {
        super.cleanup(); // Chiama la versione della classe base.
        if (this.gpuInitialized) {
            try{
                cleanupGPUResources();
                console.log("GPU resources cleaned up.");
            } catch (error){
                console.warn("Error during GPU cleanup:", error);
            }
            this.gpuInitialized = false;
        }
    }
...
// --- Modifiche a handleOptimizedProcessChunk (in simulation-worker.js) ---

/**
 *  Gestione dei messaggi nei sub-worker (versione GPU).
 *  Supporta il passaggio delle forze pre-calcolate dalla GPU.
 */
function handleOptimizedProcessChunk(message) {
    ...
    try {
        ...
        // Se abbiamo ricevuto le forze GPU, usale. Altrimenti, calcola normalmente.
        const directlyProcessedIds = message.gpuForces
            ? processInteractionPlanWithGPUForces(moleculeObjs, adaptedPlan, removedIndices, newMolecules, message.gpuForces)
...
/**
 * Processa il piano di interazione usando le forze pre-calcolate dalla GPU.
 */
function processInteractionPlanWithGPUForces(molecules, plan, removedIndices, newMolecules, gpuForces) {
    const timeScale = simulation.rules.getConstant('time_scale');
    const damping = simulation.rules.getConstant('damping');
    const now = performance.now();
    const directlyProcessedIds = new Set();

    // Applica le forze GPU *invece* di calcolare le forze da zero.
    for (const mol of molecules) {
        if (removedIndices.has(mol.id)) continue;
        directlyProcessedIds.add(mol.id);

        const force = gpuForces[mol.id];
        if (force) {
            for (let k = 0; k < 3; k++) {
                const acceleration = force[k] / mol.mass;
                mol.velocity[k] += acceleration * timeScale;
                mol.position[k] += mol.velocity[k] * timeScale;
                mol.velocity[k] *= damping;
            }
        }
    ...
    // Gestisci *solo* le reazioni (le forze sono già state applicate).
    for (const interaction of plan) {
        const mol1 = interaction.mol1;
        const mol2 = interaction.mol2;
...
// --- Modifiche a handleInitialization (in simulation-worker.js) ---
async function handleInitialization(message) {
    ...
    // Usa la nuova classe GPUOptimizedChemistry.
    simulation = new GPUOptimizedChemistry(rules, size, moleculeCount, maxNumber);
    ...
    await simulation.step(); // Esegui un passo iniziale.
...
// Sostituisci la vecchia classe con la nuova nel worker principale
onmessage = async function (event) {
  // ... (resto del codice del worker principale, come prima) ...
  // Assicurati che handleInitialization usi GPUOptimizedChemistry
   try {
        ...
        switch (message.type) {
            case 'init':
                await handleInitialization(message); // Usa la funzione modificata
                break;
